...
// calendar worker), so the stuck state is written onto OAuth-created channels directly.
const simulateCrashedSync = (calendarChannelId: string, startedAt: Date) =>
...
describe('Calendar stale-sync recovery (integration)', () => {
  ...
  it('resets a stale ongoing channel to pending and leaves a recent one running', async () => {
    await simulateCrashedSync(staleChannel.calendarChannelId, STALE_STARTED_AT);
    await simulateCrashedSync(
      recentChannel.calendarChannelId,
      RECENT_STARTED_AT,
    );

    await enqueueJob(MessageQueue.cronQueue, CalendarOngoingStaleCronJob, {});
