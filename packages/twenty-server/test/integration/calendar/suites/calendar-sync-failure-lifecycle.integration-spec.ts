...
describe('Calendar sync failure lifecycle (integration)', () => {
  ...
  it('fails the channel as unknown once the throttle attempts are exhausted', async () => {
    ...
    for (
      ...
      // The cron waits out an exponential backoff from syncStageStartedAt between
      // throttle failures; expiring it is the clock manipulation that lets the
      // next tick run immediately.
